<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="寻物启示推荐"
      url="/pages/revelation/list"
      :list="list_article"
      v-if="$check_action('/revelation/list', 'get')"
    >
	  <list_revelation :list="list_revelation"></list_revelation>
    ...
    <Card
      class="list_diy"
      title="失物招领推荐"
      url="/pages/lost_and_found/list"
      :list="list_article"
      v-if="$check_action('/lost_and_found/list', 'get')"
    >
	  <list_lost_and_found :list="list_lost_and_found"></list_lost_and_found>
    ...
    <Card
      class="list_diy"
      title="交流论坛推荐"
      url="/pages/exchange_forum/list"
      :list="list_article"
      v-if="$check_action('/exchange_forum/list', 'get')"
    >
	  <list_exchange_forum :list="list_exchange_forum"></list_exchange_forum>
...
<script>
			...
			import list_revelation from "@/components/diy/list_revelation.vue";
			import list_lost_and_found from "@/components/diy/list_lost_and_found.vue";
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*/
		get_revelation() {
			let url = "~/api/revelation/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("寻物启示" ,json.result.list);
					this.list_revelation = json.result.list;
									}
			});
		},
    			/**
		...
		*/
		get_lost_and_found() {
			let url = "~/api/lost_and_found/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("失物招领" ,json.result.list);
					this.list_lost_and_found = json.result.list;
									}
			});
		},
    			/**
		...
		*/
		get_exchange_forum() {
			...
			this.$get(url, {
					...
					if (json.result && json.result.list) {
					console.log("交流论坛" ,json.result.list);
					this.list_exchange_forum = json.result.list;
											this.list_exchange_forum.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_exchange_forum ,"exchange_forum" ,"22456");
									}
			});
		},
    				/**
		...
		get_second_hand_goods() {
			...
			this.$get(url, {
					...
					this.list_second_hand_goods = json.result.list;
						...
						this.get_praise(this.list_second_hand_goods ,"second_hand_goods" ,"24612");
									}
			});
		},
