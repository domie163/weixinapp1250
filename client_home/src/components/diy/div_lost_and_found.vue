<template>
	<view class="diy_details diy_div_lost_and_found">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','pickup_address')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>招领地址:</span>
						</view>
							<view class="diy_field diy_text">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/lost_and_found/get_obj", {
					lost_and_found_id:this.query.lost_and_found_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["lost_and_found_id"] !==0) {
