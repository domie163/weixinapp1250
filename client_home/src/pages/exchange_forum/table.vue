<template>
  <view class="page_exchange_forum diy_table diy_table_tml" id="exchange_forum_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="论坛标题" name="forum_title">
                      <uni-easyinput type="text" v-model="query.forum_title" placeholder="论坛标题" />
                    </uni-forms-item>
                            <uni-forms-item label="发布日期" name="release_date">
                      <uni-easyinput type="text" v-model="query.release_date" placeholder="发布日期" />
                    </uni-forms-item>
                </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/exchange_forum/view?" v-show="user_group == '管理员' || $check_action('/exchange_forum/table','add') || $check_action('/exchange_forum/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="1 && $check_field('get','forum_title')">
						<view class="view">
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>封面</span>
									...
									<view class="col" v-if="1 && $check_field('get','forum_label')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>论坛标签</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','post_content')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>帖子内容</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','publishing_users')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>发布用户</span>
							</view>
								...
								<span>
									{{ get_user_publishing_users(o['publishing_users']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','release_date')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>发布日期</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/exchange_forum/table','del') || $check_action('/exchange_forum/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/exchange_forum/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/exchange_forum/table','set') || $check_action('/exchange_forum/view','set') || $check_action('/exchange_forum/view','get')" class="btn btn-primary">详情</navigator>
														</view>
			</view>
		</view>
	</view>
