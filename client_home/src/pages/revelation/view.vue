          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','item_name') || $check_field('add','item_name') || $check_field('set','item_name')" label="物品名称" name="item_name">
                            <uni-easyinput type="text" v-model="form['item_name']" v-if="user_group === '管理员' || (form['revelation_id'] && $check_field('set','item_name')) || (!form['revelation_id'] && $check_field('add','item_name'))" :disabled="disabledObj['item_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','missing_address') || $check_field('add','missing_address') || $check_field('set','missing_address')" label="丢失地址" name="missing_address">
                            <uni-easyinput type="text" v-model="form['missing_address']" v-if="user_group === '管理员' || (form['revelation_id'] && $check_field('set','missing_address')) || (!form['revelation_id'] && $check_field('add','missing_address'))" :disabled="disabledObj['missing_address_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','missing_address')">
                  {{ form['missing_address'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','lost_time') || $check_field('add','lost_time') || $check_field('set','lost_time')" label="丢失时间" name="lost_time">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['revelation_id'] && $check_field('set','lost_time')) || (!form['revelation_id'] && $check_field('add','lost_time'))" @change="changeLog($event,'lost_time')" v-model="form['lost_time']" type="datetime" :disabled="disabledObj['lost_time_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','publishing_users') || $check_field('add','publishing_users') || $check_field('set','publishing_users')" label="发布用户" name="publishing_users">
                  ...
                  v-if="user_group === '管理员' || (form['revelation_id'] && $check_field('set','publishing_users')) || (!form['revelation_id'] && $check_field('add','publishing_users'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['publishing_users']"
                  :localdata="list_user_publishing_users"
              ...
              <uni-forms-item v-if="$check_field('get','details') || $check_field('add','details') || $check_field('set','details')" label="详情内容" name="details">
                    <uni-easyinput type="textarea" v-model="form['details']" v-if="user_group === '管理员' || (form['revelation_id'] && $check_field('set','details')) || (!form['revelation_id'] && $check_field('add','details'))" :disabled="disabledObj['details_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/revelation/upload?'),
    ...
    async get_list_item_status() {
          ['未找回','已找回'].map((o) => this.list_item_status.push({value:o,text:o}));
            },
