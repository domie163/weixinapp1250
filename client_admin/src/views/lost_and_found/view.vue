	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="物品名称" prop="item_name">
							  ...
							  v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','item_name')) || (!form['lost_and_found_id'] && $check_field('add','item_name'))" :disabled="disabledObj['item_name_isDisabled']"></el-input>
				...
				<el-form-item label="物品图片" prop="item_picture">
						...
						:show-file-list="false" v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','item_picture')) || (!form['lost_and_found_id'] && $check_field('add','item_picture'))">
					...
					<el-image v-else-if="$check_field('get','item_picture')" style="width: 100px; height: 100px"
						...
						<div slot="error" class="image-slot">
							<img src="../../../public/img/error.png" style="width: 90px; height: 90px" />
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','pickup_address') || $check_field('add','pickup_address') || $check_field('set','pickup_address')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="招领地址" prop="pickup_address">
												<el-input id="pickup_address" v-model="form['pickup_address']" placeholder="请输入招领地址"
							  v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','pickup_address')) || (!form['lost_and_found_id'] && $check_field('add','pickup_address'))" :disabled="disabledObj['pickup_address_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','pickup_address')">{{form['pickup_address']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="发布用户" prop="publishing_users">
							...
							<!--v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','publishing_users')) || (!form['lost_and_found_id'] && $check_field('add','publishing_users'))" :disabled="disabledObj['publishing_users_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','publishing_users')">{{form['publishing_users']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','publishing_users')) || (!form['lost_and_found_id'] && $check_field('add','publishing_users'))" id="publishing_users" v-model="form['publishing_users']" :disabled="disabledObj['publishing_users_isDisabled']">
				...
				<el-form-item label="详情内容" prop="details">
						...
						v-if="user_group === '管理员' || (form['lost_and_found_id'] && $check_field('set','details')) || (!form['lost_and_found_id'] && $check_field('add','details'))" :disabled="disabledObj['details_isDisabled']"></el-input>
