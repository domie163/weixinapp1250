<template>
  <view class="page_lost_and_found diy_table diy_table_tml" id="lost_and_found_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/lost_and_found/view?" v-show="user_group == '管理员' || $check_action('/lost_and_found/table','add') || $check_action('/lost_and_found/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>物品图片</span>
									...
									<view class="col" v-if="1 && $check_field('get','pickup_address')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>招领地址</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','item_status')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>物品状态</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/lost_and_found/table','del') || $check_action('/lost_and_found/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/lost_and_found/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/lost_and_found/table','set') || $check_action('/lost_and_found/view','set') || $check_action('/lost_and_found/view','get')" class="btn btn-primary">详情</navigator>
														</view>
			</view>
		</view>
	</view>
		...
		data() {
			return {
                ...
                list_item_type: [{value:"",text:"全部"}],
                                            // 物品状态列表
                list_item_status: [{value:"",text:"全部"}],
                                        // 用户列表
